...
SELECT * FROM AddressBook
DELETE FROM AddressBook WHERE FIRST_NAME = 'GOWTHAM'
...
--PROCEDURE CREATION
CREATE PROCEDURE ADD_PERSON (@FIRST_NAME VARCHAR(20), @LAST_NAME VARCHAR(20), @ADDRESS VARCHAR(50), @CITR VARCHAR(20), @STATE VARCHAR(20), @PIN INT, @PHONE FLOAT, @EMAIL VARCHAR(40), @CONTACT_TYPE VARCHAR(20))
AS
INSERT INTO AddressBook VALUES (@FIRST_NAME, @LAST_NAME, @ADDRESS, @CITR, @STATE, @PIN, @PHONE, @EMAIL, @CONTACT_TYPE)
...
SELECT CONTACT_GROUP, COUNT(FIRSE_NAME) FROM CONTACT_DETAILS CD LEFT JOIN CONTACT_TYPE CT ON CD.CONTACT_ID = CT.CONTACT_ID GROUP BY CT.CONTACT_GROUP
SELECT CONTACT_GROUP, COUNT(FIRSE_NAME) FROM CONTACT_DETAILS CD RIGHT JOIN CONTACT_TYPE CT ON CD.CONTACT_ID = CT.CONTACT_ID GROUP BY CT.CONTACT_GROUP

--SUB QUERY
SELECT FIRSE_NAME FROM CONTACT_DETAILS WHERE CONTACT_ID = (SELECT CONTACT_ID FROM PHONE_NUMBER WHERE PRIMARY_NUMBER = 7894561230)

--INDEX
--SINGLE INDEX
CREATE INDEX EMAIL_INDEX ON CONTACT_DETAILS (EMAIL)
SELECT EMAIL FROM CONTACT_DETAILS WHERE FIRSE_NAME = 'NANTHAGOPAL'
--UNIQUE INDEX (DUPLICATES NOT ALLOWED)
CREATE UNIQUE INDEX UNIQUE_NAME ON CONTACT_DETAILS (FIRSE_NAME)
INSERT INTO CONTACT_DETAILS VALUES ('NANTHAGOPAL','SHANMUGAM','NO 1 - COIMBATORE','COIMBATORE','TAMILNADU',656598,'nantha@gmail.com')
INSERT INTO CONTACT_DETAILS VALUES ('GOPAL','SHANMUGAM','NO 1 - COIMBATORE','COIMBATORE','TAMILNADU',656598,'nantha@gmail.com')
--COMPOSITE INDEX
CREATE INDEX COMPOSITE_INDEX ON CONTACT_DETAILS (LAST_NAME,ADDRESS,CITY)
